...
export function TeamsGrid({
  ...
  if (!teamsItems || teamsItems.length === 0) {
    return (
  ...
  return (
    <div className="container mx-auto p-3 lg:p-8">
      ...
      {/* Teams Grid - 1 column on mobile, 2 on md, 3 on lg, 4 on xl */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
        {teamsItems.map((item, index) => {
          const hero = item.hero as Media | undefined
            ...
            >
              <div className="relative w-full min-h-[300px] overflow-hidden rounded-lg bg-card border border-border hover:shadow-lg transition-all duration-300 group-hover:scale-[1.02]">
                <div className="relative w-full h-[200px]">
                  <Image
                    src={heroUrl}
                    alt={item.name || hero.alt || `Team ${index + 1}`}
                    fill
